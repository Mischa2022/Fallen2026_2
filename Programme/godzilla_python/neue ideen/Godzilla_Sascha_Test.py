...
def create_menu():
    ...
    label1 = tk.Label(root2, text="Modus wählen")
    label1.pack()

    schaltf1 = tk.Button(root2, text="Checkin",command=create_gui1)
    #mode = 1
    schaltf1.pack()
    schaltf2 = tk.Button(root2, text="Godzilla",command = create_gui2)
    #mode =2
    schaltf2.pack()

    
def add_background_image(master):
    master.bg_image = tk.PhotoImage(file="background.png")  # Persist image
    bg_label = tk.Label(master, image=master.bg_image)
...
def create_gui1():
    global output_text, scrollbar, button_frame, radio_frame, radio_var, root,mode
    root = tk.Tk()
    #mode=1
    ...
    window = tk.Toplevel()  # or root = tk.Tk()
    add_background_image(window)
