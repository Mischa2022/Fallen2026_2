...
bool redhead(int range, int rounds){
    int gamble[rounds];
    int coin_minus = 0;
    int coin_plus = 0;
    int edge= rounds/2;
    bool wurfergebnis =false;
      
    for ( int x=0;x<rounds;x++){
    gamble[x]=random(1,range);
    if(gamble[x]<edge){coin_minus=coin_minus+1;}
    else if(gamble[x]>edge){coin_plus=coin_plus+1;}
  /*
    Serial.print("Gamble: ");
    Serial.print(gamble[x]);
    Serial.print("|coin_minus: ");
    Serial.print(coin_minus);
    Serial.print("|coin_plus: ");
    Serial.print(coin_plus);
    Serial.println();
    */
    delay(100);
