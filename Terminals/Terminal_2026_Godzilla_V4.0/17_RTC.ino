...
int unix_converter(unsigned long t_unix_date_start,int Zeitzone){
  ...
  //char* typen3[]{" Grundzeit"," Winterzeit"," Sommerzeit","Leer"};
...
if (t_unix_date==0){
 ...
 /*   if(port==1){
  Serial1.printf("Date: %4d-%02d-%02d %02d:%02d:%02d", year(t_unix_date), month(t_unix_date), day(t_unix_date), hour(t_unix_date), minute(t_unix_date), second(t_unix_date));

  }*/
    if(port==2){
  Serial2.printf("Date: %4d-%02d-%02d %02d:%02d:%02d", year(t_unix_date), month(t_unix_date), day(t_unix_date), hour(t_unix_date), minute(t_unix_date), second(t_unix_date));

  }
    if(port==3){
  Serial3.printf("Date: %4d-%02d-%02d %02d:%02d:%02d", year(t_unix_date), month(t_unix_date), day(t_unix_date), hour(t_unix_date), minute(t_unix_date), second(t_unix_date));
