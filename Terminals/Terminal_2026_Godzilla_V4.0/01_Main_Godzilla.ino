...
void loop() {
  ...
  result = rotary.process();
      ...
      if(modus==2||modus==20){
        if(modus==20){modus=2;}
        if(Anzeige_update ==true){maxebene=0;}
        if(Anzeige_Nanoclean ==true){maxebene=1;}
        if(Anzeige_Dekon ==true){maxebene=2;}
        if(Anzeige_Lifecleaner ==true){maxebene=3;}
        if(Anzeige_Blutwaesche ==true){maxebene=4;}
        if(Anzeige_Cleanray ==true){maxebene=5;}
         if(Anzeige_drawback ==true){maxebene=6;}
         if(Anzeige_redhead ==true){maxebene=7;}
