...
int medibuchung(unsigned long medikament){
...
//welches Medikament soll gebucht werden und welche Spezifikationen hat es
            ...
            if(medikament==8){
              Tag_analyse(1,0);
             // redhead=Ray_value_extern_read/2
             // Drawback=random(Drawback_min_effekt,Drawback_max_effekt)*1000UL;
              Bonus=0;
              limit_tag= 100; 
              limit_con= 100;
...
//2. Einnahme Medis erlaubt          
     ...
     if(used_tag<limit_tag&&used_con<limit_con){ // muss kleiner sein, damit eine weitere buchung erlaubt ist
        ...
        //ja   
               ...
               myprint("Aktuelle Zeit:","",aktuell_time,true,true,false,1,port);   

              
               myprint("Letzte Buchung:","",Last_Booking_read,true,true,false,1,port);   
...
///////////////////////////////////////////////////////////////////////////////////////
                  ...
                  if(LifeCleaner_effekt_read>=aktuell_time){
                      ...
                      myprint("Lifecleanereffekt aktiv: ja","","",true,true,false,1,port);
                      delta=0;
                      ...
                      myprint("Lifecleanereffekt aktiv: nein","","",true,true,false,1,port);
                      if(aktuell_time-Last_Booking_read<0){
                        delta=0;
                          ...
                          if(Last_Booking_read<LifeCleaner_effekt_read){
                            ...
                            myprint("Aufschlag nach Lifecleaner","","",true,true,false,1,port);
                          }
                          else{
                          delta=aktuell_time-Last_Booking_read;

                          myprint("Aufschlag nach letzter Buchung","","",true,true,false,1,port);
                          }
                         }
 ...
 ///////////////////////////////////////////////////////////////////////////////////////
              ...
              myprint("Raywert alt:","",Ray_value_extern_read,true,true,false,1,port);
              

              myprint(Medi[medikament-1],"","",true,true,false,1,port);
        
              if(medikament!=7){
        
              myprint("Bonus:-","",Bonus,true,true,false,1,port);
              
              //deltaray=(tagesdosis*delta/86400)*1000;
              deltaray=(tagesdosis*delta*10)/864;
              myprint("Delta in RAY:+","",deltaray,true,true,false,1,port);
              ...
              myprint("Malus:+","",Bonus,true,true,false,1,port);
            ...
            myprint("Raywert neu:","",ray_writing,true,true,false,1,port);
             ...
             if(medikament==8){
                ...
                if(ray_writing>=redhead_limit){
                myprint("Raywert neu:","",ray_writing/2,true,true,false,1,port);
                writing(knownKeys[last_key],58,Con_Start_read,time_writing);
                writing(knownKeys[last_key],56,Con_End,ray_writing/2);
                Serial.print("Behandlung gestartet: "); 
                redhead(6,20);
                }
                else{
                myprint("Raywert neu:","",ray_writing,true,true,false,1,port);
...
//nein 
              ...
              if(used_tag>=limit_tag){
                ...
                myprint("Tageslimit erreicht","","",true,true,false,1,port);
              ...
              if(used_con>=limit_con){
                ...
                myprint("Conlimit erreicht","","",true,true,false,1,port);
                buchungsgrund2=" Limit erreicht";
              }
 //keine Medibuchung nur Update             
          ...
          //myprint("------------------------------------------------","","",true,true,false,1,port);    
              ...
              if(LifeCleaner_effekt_read>=aktuell_time){
                ...
                myprint("Lifecleanereffekt aktiv: ja","",aktuell_time,true,true,false,1,port);   
                delta=0;
                time_writing=LifeCleaner_effekt_read;
              }else {
            
                myprint("Lifecleanereffekt aktiv: nein","",aktuell_time,true,true,false,1,port);   
                 if(aktuell_time-Last_Booking_read<0){
                        delta=0;
                      }
                        else{
              ...
              myprint(Medi[0],"","",true,true,false,1,port);   
          
              myprint("Bonus:","",0,true,true,false,1,port); 
...
//, mach nichts
        ...
        myprint(buchungsgrund,"","",true,true,false,1,port);  
         buzzer_play( 3,100);
   }
