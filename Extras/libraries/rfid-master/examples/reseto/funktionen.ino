...
int ray_manipulation(unsigned long wert){
   unsigned long schreibwert=0;
 // Serial.println("Raywert ändern gestartet");
  ...
  digitalWrite(greenPin,HIGH); 
      ...
      schreibwert=wert+Ray_value_extern_read;
      
      if(schreibwert>450000){
        schreibwert=450000;
        } // abfangen von zu großen werten

      if(schreibwert<10000){
        schreibwert=10000;
        } // abfangen von zu kleinen werten
        
      writing(knownKeys[key_used],56,Ray_value_intern_read,schreibwert); 
    //  Tag_analyse(1,0); //für ausgabe der Änderung
    //  msg4="Raywert";
    //  msg5=" ändern fertig";

    Serial.print("geschrieben:");Serial.println(schreibwert);
      ergebnisA=0;
      ergebnisB=0;
}
 ...
 */
  ...
  //  Serial.write("Key A trailorblock "); // Ausgabe ist für Test nicht mehr nötig
  ...
  if (status != MFRC522::STATUS_OK) {
  //  Serial.write("Lesen Block "); // Ausgabe ist für Test nicht mehr nötig
  //  Serial.print(block);
  //  Serial.write(" nicht ok");
    return 1;
  }
