...
const char *entry1alt[] {
  "On",
  "Off",
};

constexpr byte maxOptions = 3;
...
byte level = 0;                       // the actual hierachy level of menu

byte option[noOfEntries];
byte menu1 = 0;

// return the total amount of valid options for entry level 1
byte getEntries1(byte actual)
{
  for (byte i = 0; i < maxOptions; i++)
  {
    if (!strcmp(entry1[actual][i], "")) return i;
  }
  return maxOptions;
}
...
void readButton()
{
  if (digitalRead(downButton) == LOW) {
    ...
    {
      if (menu1 < getEntries1(menu) - 1) menu1++;
      updateOption(menu);
    }
...
void updateOption(byte actual)           // displays the level1 options
{
  lcd.clear();
  byte from = 0;
  from = (menu1 / rows) * rows;         // the first entry on the LCD is calculated depending on the available LCD rows and the current entry
  byte to = from + rows - 1;            // the last entry depends on the available rows on the LCD
  if (to >= getEntries1(menu)) to = getEntries1(menu) - 1;
  byte rowActual = 0;
  for (byte i = from; i <= to; i++)
  {
    lcd.setCursor(0, rowActual);
    if (i == menu1) lcd.write('>');            // marker to indicate the actual menu entry
    lcd.setCursor(1, rowActual);
    lcd.print(entry1[actual][i]);
    if (i == option[actual]) lcd.write('!');   // mark the currently set option
    rowActual++;
  }
}
