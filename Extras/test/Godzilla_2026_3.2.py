...
logging.basicConfig(filename="godzilla.log",level=logging.INFO,format="%(asctime)s - %(message)s")
...
class SerialManager:
    ...
    def connect(self):
        try:
            ...
            self.connected = True
            logging.info("Verbunden mit %s", self.port)
        except:
            self.connected = False
    ...
    def send(self, command):
        try:
            if self.ser and self.ser.is_open:
                ...
                logging.info("Gesendet: %s", command.strip())
        except:
            self.connected = False
...
class GodzillaApp:
    ...
    def create_ui(self):
        ...
        for tab in TAB_CONFIG.keys():
            ttk.Radiobutton(
                tab_frame,text=tab,value=tab,variable=self.tab_var,
    ...
    def create_button_style(self, text):
        ...
        self.style.map(
            style_name,
            background=[("active", color_config.get("hover_bg", color_config["bg"]))],
            foreground=[("active", color_config["fg"])]
        )
        return style_name
    ...
    def update_buttons(self):
        ...
        for i, label in enumerate(config["labels"]):
            ...
            ttk.Button(
                self.button_frame,text=label,style=style_name,
                command=lambda cmd=config["commands"][i]: self.send_command(cmd)
    ...
    def create_manual(self):
        ...
        for b in buttons:
            ...
            ttk.Button(self.button_frame,text=b,style=style_name,command=action).grid(row=row,column=col,padx=6,pady=6,sticky="nsew")
            col+=1
    ...
    def toggle_dark_light(self):
        if self.dark_mode:
            self.bg_color = "white"; self.fg_color="black"; self.dark_mode=False
        else:
            self.bg_color="#1E1E1E"; self.fg_color="white"; self.dark_mode=True
        self.root.configure(bg=self.bg_color)
        self.update_buttons()
...
class PortSelector:
    ...
    def __init__(self):
        ...
        self.combo = ttk.Combobox(self.root,values=list(self.port_map.keys()),state="readonly",width=50)
        self.combo.pack(pady=10)
        if self.port_map: self.combo.current(0)
        ttk.Button(self.root,text="Start GODZILLA 3.2",command=self.start_app).pack(pady=20)
        self.root.mainloop()
