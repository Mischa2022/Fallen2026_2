...
//2x Prozess
  ...
  if(ausfuehrlich==1){  
    ...
    myprint("--------------------------------------------","","",true,true,false,1,port);
    myprint("Ausführliche Analyse gestartet","","",true,true,ausfuehrlich,1,port);
  }
...
// für aktuell berechnete werte des raywerts
if(raywert!=0){
  //prozess();
  msg4="Ray:";
        ...
        if(aktuell_time<=con_start_time){
          ...
          myprint("zu früh für berechnung","","",true,true,false,1,port);
          ray_writing2=(Ray_value_extern_read+0)-0;
        }
        else if(aktuell_time>con_start_time+rest_time+1+((anzahl_tage-1)*86400)){
     
          myprint("viel zu spät","","",true,true,false,1,port);
          ray_writing2=500000;
          delta2=0;
        }
        else if(Con_Start_read!=con_start_time){
            myprint("keine Berechnung möglich","","",true,true,false,1,port);
          ray_writing2=(Ray_value_extern_read+(0))-0;
          delta2=0;
        }
        else{ //im Spiel
        ...
        if(LifeCleaner_effekt_read>=aktuell_time){
                    ...
                    //Serial.print(F("Delta2A:");Serial.println(delta2);
