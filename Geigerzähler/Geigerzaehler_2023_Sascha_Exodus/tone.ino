...
int  melodyPin=buzzer_pin;
  ...
  if (song == 2) {
 //  
  int notes = sizeof(godmelody) / sizeof(godmelody[0]) / 2;
  int wholenote = (60000 * 4) / godtempo;
  int divider = 0, noteDuration = 0;
  
    for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {

    // calculates the duration of each note
    divider = godmelody[thisNote + 1];
    if (divider > 0) {
    ...
    tone(melodyPin, godmelody[thisNote], noteDuration * 0.9);
