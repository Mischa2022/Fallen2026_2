...
//*************************************************************************************//
  ...
  if(variante==1||variante==10){
        ...
        myprint("Con-Start:","",Con_Start_read,true,true,ausfuehrlich,1,0);
        myprint(schrift[0],"",Last_Booking_read,true,true,ausfuehrlich,1,0);
    ...
    if(archtype_read==0){
      typ=0;
        ...
        myprint(schrift[1],typen[typ],"",true,true,false,1,0); 
        myprint(schrift[2],"",LifeCleaner_effekt_read,true,true,ausfuehrlich,1,0);      
        myprint(schrift[3],"",Ray_value_extern_read,true,true,false,1,0);
        myprint("Con Ende:","",Con_End_read,true,true,ausfuehrlich,1,0);
  ...
  if(variante==2||variante==10){
    for(int y = 0; y <= 7; y++) {
      if(NanoClean_read[y]!=0){
        myprint(schrift[y+4],"",NanoClean_read[y],true,true,ausfuehrlich,1,0);
      }
    }
  }
  ...
  if(variante==3||variante==10){
    for(int y = 0; y <= 13; y++) {
      if(Dekon_read[y]!=0){
        myprint(schrift[y+12],"",Dekon_read[y],true,true,ausfuehrlich,1,0);
      }
    }
  }
  ...
  if(variante==4||variante==10){
    for(int y = 0; y <= 9; y++) {
      if(LifeCleaner_read[y]!=0){
        myprint(schrift[y+26],"",LifeCleaner_read[y],true,true,ausfuehrlich,1,0);
      }
    }
  } 
  ...
  if(variante==5||variante==10){ 
    for(int y = 0; y <= 9; y++) {
      if(BloodClean_read[y]!=0){
        myprint(schrift[y+36],"",BloodClean_read[y],true,true,ausfuehrlich,1,0);
      }
    }
  }
  ...
  if(variante==6||variante==10){
    for(int y = 0; y <= 7; y++) {
      if(Cleanray_read[y]!=0){
        myprint(schrift[y+46],"",Cleanray_read[y],true,true,ausfuehrlich,1,0);
      }
    }
  }
...
*/
 ...
 if(error==0){
  ...
  if(rights>0&&rights<11112){
    myprint("Rechte:NBCLD","","",true,true,false,1,0);
    myprint("       ","","",true,false,false,1,0);

  if(rights<10000){
  
  myprint("0","","",true,false,false,1,0);}
  if(rights<1000){
  ...
  myprint("0","","",true,false,false,1,0);}
  myprint("","",rights,true,true,false,1,0);

  }
    else{
          myprint("Rechte: keine","","",true,true,false,1,0);
  ...
  myprint("------------------------------------------------","","",true,true,false,1,0);
    if(variante==2||variante==10){
      if(Nanocounter_allday>0){
          myprint(schrift[54],typen2[0],Nanocounter_thisday,true,false,false,1,0);
          myprint(schrift[55],typen2[1],Nanocounter_allday,true,true,false,1,0);
        }
      }
        
    if(variante==3||variante==10){
      if(Dekoncounter_allday>0){
          myprint(schrift[56],typen2[0],Dekoncounter_thisday,true,false,false,1,0);
          myprint(schrift[57],typen2[1],Dekoncounter_allday,true,true,false,1,0);
        }
      }
      
    if(variante==4||variante==10){
      if(Lifecounter_allday>0){
          myprint(schrift[58],typen2[0],Lifecounter_thisday,true,false,false,1,0);
          myprint(schrift[59],typen2[1],Lifecounter_allday,true,true,false,1,0);
      }
      }

    if(variante==5||variante==10){   
      if(Bloodcounter_allday>0){
          myprint(schrift[60],typen2[0],Bloodcounter_thisday,true,false,false,1,0);
          myprint(schrift[61],typen2[1],Bloodcounter_allday,true,true,false,1,0);
        }
      }
       
    if(variante==6||variante==10){  
      if( Cleancounter_allday>0){
          myprint(schrift[62],typen2[0],Cleancounter_thisday,true,false,false,1,0);
          myprint(schrift[63],typen2[1],Cleancounter_allday,true,true,false,1,0);
        }
      }
