...
unsigned long Redhead;// variable fuer Ray-Aufschlag Einnahme Redheadbehandlung
unsigned long redhead_limit =399000; //Grenzwert ab wann Redheadbehandlung greift
unsigned long coin_wert;
...
char* Medi[] {"Update     ","Nanoclean  ","Dekon      ","Lifecleaner","Bloodclean ","Cleanray   ","Drawback   ","Redhead    "};
...
//******************************************************************                                                     
     ...
     // Serial.write('\n');delay(write_pause);
      ...
      Serial.print(F("Con_End:"));Serial.println(Con_End);
      
   if(aktuell_time>con_start_time+rest_time){
      //Serial.print(F("vergangene Sekunden ab Anreise:"));
      //Serial.print(aktuell_time-(con_start_time+rest_time));
      Serial.print(F("vergangene Stunden seid Anreise:"));
...
delay(50);
  ...
  Tag_analyse(1,0); 
        ...
        if(msg5=="kein Ray    "){digitalWrite(red_pin,HIGH);digitalWrite(green_pin,LOW);digitalWrite(blue_pin,HIGH);}
        if(msg5=="light Green "){digitalWrite(red_pin,LOW);digitalWrite(green_pin,HIGH);digitalWrite(blue_pin,LOW);}  
        if(msg5=="Green       "){digitalWrite(red_pin,LOW);digitalWrite(green_pin,HIGH);digitalWrite(blue_pin,LOW);}  
        if(msg5=="dark Green  "){digitalWrite(red_pin,LOW);digitalWrite(green_pin,HIGH);digitalWrite(blue_pin,LOW);}  
        if(msg5=="light Yellow"){digitalWrite(red_pin,HIGH);digitalWrite(green_pin,HIGH);digitalWrite(blue_pin,LOW);}
        if(msg5=="Yellow      "){digitalWrite(red_pin,HIGH);digitalWrite(green_pin,HIGH);digitalWrite(blue_pin,LOW);}
        if(msg5=="dark Yellow "){digitalWrite(red_pin,HIGH);digitalWrite(green_pin,HIGH);digitalWrite(blue_pin,LOW);}
        if(msg5=="light Red   "){digitalWrite(red_pin,HIGH);digitalWrite(green_pin,LOW);digitalWrite(blue_pin,LOW);}
        if(msg5=="Red         "){digitalWrite(red_pin,HIGH);digitalWrite(green_pin,LOW);digitalWrite(blue_pin,LOW);}
        if(msg5=="dark Red    "){digitalWrite(red_pin,HIGH);digitalWrite(green_pin,LOW);digitalWrite(blue_pin,LOW);}
        if(msg5=="kein Spiel  "){digitalWrite(red_pin,LOW);digitalWrite(green_pin,LOW);digitalWrite(blue_pin,HIGH);}
    
      Serial.print("Hallo:");Serial.println(msg5);
